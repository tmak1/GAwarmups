...
const splitTheBill = group => {
    ...
    eachToPay = Math.round(((total/Object.keys(group).length) + Number.EPSILON) * 100) / 100; 
    Object.entries(group).forEach(([person, paid]) => {
        newGroup[person] = eachToPay - paid;
    }) 
    return newGroup;  
}

result = splitTheBill(group);
console.log(result);
